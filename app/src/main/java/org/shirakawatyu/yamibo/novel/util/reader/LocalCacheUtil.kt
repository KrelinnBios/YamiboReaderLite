...
class LocalCacheUtil(private val context: Context) {
    ...
    suspend fun clearOlderThan(cutoffTimestamp: Long): Int = withContext(Dispatchers.IO) {
        try {
            val currentIndex = _index.value
            val newIndex = currentIndex.toMutableMap()
            var removedCount = 0

            currentIndex.forEach { (novelUrl, novelCache) ->
                val retainedPages = novelCache.pages.toMutableMap()
                novelCache.pages.forEach { (pageNum, pageInfo) ->
                    if (pageInfo.timestamp < cutoffTimestamp) {
                        File(getCacheDir(), getCacheFileName(novelUrl, pageNum)).delete()
                        retainedPages.remove(pageNum)
                        removedCount++
                    }
                }
                ...
                if (retainedPages.isEmpty()) {
                    newIndex.remove(novelUrl)
                } else if (retainedPages.size != novelCache.pages.size) {
                    newIndex[novelUrl] = novelCache.copy(pages = retainedPages)
                }
            }

            if (removedCount > 0) writeIndex(newIndex)
            removedCount
        } catch (e: Exception) {
            Log.e(LOG_TAG, "清理过期缓存失败", e)
            0
        }
    }
