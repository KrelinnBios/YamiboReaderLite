...
- 论坛功能：登录浏览百合会论坛，支持自动签到、深色显示、简繁切换、网络优化、帖子直达与内容屏蔽。
- 漫画功能：浏览和搜索漫画板块、管理本地目录，并通过原生阅读器切换章节、缓存图片和调整阅读方式。
- 小说阅读：支持排版与翻页调节、正文图片、简繁转换、跨页章节目录、进度记录和页面缓存。
- 收藏管理：同步论坛收藏，提供分类搜索、排序置顶、批量管理、缓存清理和内容更新提醒。
- 历史记录：记录浏览过的帖子，支持日期筛选、组合搜索、批量删除和独立页面打开。
- 数据维护：统计与清理小说、漫画缓存，并导入导出收藏、目录、进度、历史、黑名单及设置。
- 应用服务：提供新手引导、集中设置，以及 GitHub Releases 更新检查、下载和安装。
