...
本文件适用于整个仓库，作为 AI 编码代理修改 YamiboReaderLite 时的唯一项目规范。除非用户另有要求，使用中文回复和说明。

## 项目状态

YamiboReaderLite 已结束维护，本仓库用于保留项目最终实现状态。这是理解本仓库的最高优先级前提。

- 不再承担持续开发、功能迭代、架构演进或依赖现代化任务；不要根据“以后可能继续维护”的假设提出改造方案。
- 默认把当前源码视为最终快照，不主动提出新增功能、大规模重构、依赖升级或“顺便清理”。
- GitHub 是否临时取消 Archive 仅影响仓库能否写入，不改变项目已经结束维护的事实。
- README 顶部停止维护说明属于项目最终状态，应保留；其中指向 YamiboPlus 的文字只是面向用户的后续去向提示，不代表两个仓库存在代码同步、兼容、回移植或迁移关系。
- 如果用户明确要求再次修改本仓库，以该次明确要求为准；仍优先采用范围小、风险低、可验证的实现。
...
只有用户明确要求再次修改本仓库时才进入实际修改流程。

- 先读取与任务直接相关的实现、调用链和已有测试，不根据文件名或界面表象猜测根因。
- 涉及论坛解析、CSS、JavaScript、网络、WebView 或真实设备行为时，优先获取真实页面或可复现样本。
- 涉及网络、目录、缓存、屏蔽、链接直达、更新等历史敏感模块时，先阅读本文件“已确立的决定”。
- 先检查工作区已有修改；无关修改不要清理、回滚或格式化。
- 如果任务只是代码审阅、历史追溯或功能解释，不修改仓库，也不额外提出未来重构路线。

## 修改原则

- 优先沿用现有类、状态流和工具函数，不在 Composable 中复制网络、缓存、解析或持久化逻辑。
- 以最小 diff 为原则，不把一次定向修改扩展成架构清理、全局格式化或无关功能调整。
- 不顺手升级 Gradle、AGP、Kotlin、Compose、OkHttp、Retrofit 等依赖。
- 不为了“现代化”引入新的 DI 框架、数据库、模块化方案或导航框架。
- 不为了拆文件而重写已经稳定工作的页面、状态链路或大文件。
- 不提交构建产物、签名材料、`.env`、`local.properties`、临时 HTML、抓包文件、设备日志或其他无关生成物。
- 修改论坛解析、CSS 或 JavaScript 时使用真实页面样本和精确条件，不用大范围 selector 或 regex 猜测所有模板。

## 代码地图

主源码位于 `app/src/main/java/org/shirakawatyu/yamibo/novel/`。

- `MainActivity.kt`：主导航和大量页面级协调逻辑。文件很大，这是最终快照的一部分，不应仅为了拆文件进行结构重写。
...
- 漫画发现：浏览和搜索中文漫画区、漫画图源区，生成、更新和管理本地目录。
...
- WebView 暂停期间 `evaluateJavascript` 可能丢失。恢复路径（如 `resumeBbsWebViewAfterChildPage`、`resumeMineWebViewAfterChildPage`）需要重新注入主题和页面交互 JavaScript。
...
- 漫画收藏版块固定 fid 30「中文百合漫画区」、fid 37「百合漫画图源区」；小说版块固定 fid 49「文學區」、fid 55「轻小说/译文区」、fid 60「TXT小说区」。未识别收藏允许手动选小说、漫画或其他；选择其他后从收藏页移出。
...
- 论坛屏蔽数据在 `ForumBlocklistManager`；注入在 `PageJsScripts.getForumBlockerJs`；JavaScript ↔ 原生桥为 `ForumBlocklistJSInterface`（`AndroidForumBlocklist`）；管理 UI 为 `ForumBlocklistDialog`。
...
以下内容用于解释最终代码为什么这样写。它们通常来自真实论坛页面、实机问题或已经发生过的回归，不应仅凭通用最佳实践判断为多余逻辑。
...
- 深色论坛规则集中在 `util/theme/DarkClassic.kt`；HTML 代理注入与运行时 JavaScript 注入复用同一份 CSS。
...
- 底栏单击 = 切换板块；已经在本板块时不重载。长按 = 回该板块主页，通过 `returnToHome(notifyHome = true)` / `goHomeEvent`。
...
- 自动归并默认依据作品名 + 汉化组，不默认按发布账号拆目录。只有标题明确表示个人 / 非固定团队发布（`MangaTitleCleaner.isIndividualRelease`）时才用发布者兜底；用户手动发布者设置始终优先保留。
...
- 汉化组过滤是硬过滤，统一复用 `DirectoryRepository.filterChaptersByDirectoryConstraints`。
- 首楼如果提供编号式跨帖子链接列表，视为权威目录：按列表顺序和标题保存，`MangaDirectory.authoritativeLinks` 标记；不自动搜索扩展未列出的帖子。`manuallyUpdateDirectory` 对权威目录整体跳过，列表只在打开原帖时按首楼刷新。
...
- 只屏蔽别人的内容。当前登录 uid 与作者 uid 相同的主题 / 楼层（含一楼）不显示“屏蔽”。`view=me` 或自己的 `mod=space&do=thread/reply/favorite` 列表整页跳过。
...
如果只是阅读或审阅仓库，无需执行构建。只有用户明确要求再次修改代码时，按修改范围选择验证。
...
- 修改 URL 归一化、Cookie、会话、HTML 解析、阅读器返回链接、图片策略、应用更新解析、`YamiboPostLinkUtil`、`ForumBlocklistManager` 等已有测试覆盖的模块时，同时运行 `testDebugUnitTest`，并按需要补充或更新 `app/src/test` 下的测试。
- 纯文档修改无需运行 Gradle。
- 增量构建出现 `TaskOutputsBackup` / `IOException` 等缓存损坏时再使用 `clean assembleDebug`，不要把 `clean` 当默认命令。
- UI、WebView 生命周期、论坛模板、网络恢复和系统安装行为无法由 JVM 测试完整覆盖，需要真实设备或页面样本才能确认。
- 无法完成实机或真实页面验证时，明确说明剩余验证项，不直接宣称未验证行为已经修复。

## CI 与发布快照

以 `.github/workflows/ci.yml` 和 `.github/workflows/build-apk.yml` 的当前内容为准。本节只记录最终仓库中这两条流程的语义，不为未来版本升级制定规则。
...
- 项目已经结束维护，因此这些流程是最终仓库快照的一部分，不代表未来仍计划发布版本。

## 文档同步

- README 顶部停止维护警告属于项目最终状态，不在普通文档整理中删除或弱化。
- README 面向用户描述 Lite 最终版本已经存在的能力，不写未来路线图。
- README 主体结构维持：项目简介 → 功能概览 → 界面预览 → 使用方式 → 隐私与数据 → 内容边界 → 许可协议。
- 结束维护项目不保留“反馈与贡献”作为默认章节，不继续承诺处理功能建议、兼容性问题或新版本请求。
...
- 一个提交聚焦一个明确主题。
- 提交信息沿用仓库现有简短中文风格，直接说明实际结果。
- 不混入无关格式化、构建产物或其他工作区修改。
- 不覆盖、回滚或整理用户已有的无关改动。

## 回答本仓库相关问题时

- 把 YamiboReaderLite 当作已经完成并停止维护的独立历史项目讨论。
