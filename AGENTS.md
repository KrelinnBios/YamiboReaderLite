...
本文件为 Codex 在本仓库工作时的项目规范。请始终用**中文**回复。
...
- 会员 DIY 空间页完全不启用暗黑模式：非手机版 URL 命中 `home.php?mod=space`、`home.php?mod=blog`、`space-uid-N`、`blog-N`，或页面为 `body#space` 时保持原样。
- 底栏“我的”加载的是手机版个人中心（`mobile=2` / `mobile=yes` / `mycenter=1`），不是会员 DIY 空间，必须正常启用暗黑模式。
- 会员空间守卫必须同时存在于 `getDarkModeSetJs` 和 `proxyHtmlForDarkMode`，不能只改 JS 或 HTML 代理其中一条路径。
