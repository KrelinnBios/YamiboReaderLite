...
- 已存目录名如果是旧版清洗器留下的残次品，`initDirectoryForThread` 会自动迁移：`isStaleCleanBookName` 处理"切多了"（如"作品名 52+"），`isTruncatedCleanBookName` 处理"切少了"（如集合后缀识别上线前把"作品名短篇集"截成"作品名"）。两者都要求新旧名字之间是纯後缀/残渣关系，用户手动改的名字不会被误还原。
...
- README「项目简介」结尾补一句范围说明（如"以下说明仅描述 YamiboReaderLite 当前实际提供的功能"），不要只留功能段落，也不要和「数据与安全」小节重复表述。
- 顶部 logo 统一放在 `icon/logo.svg`（相对仓库根目录）。
